...
## Ek: Stok yönetimi ekranları (kullanıcının kendi envanter ürünü, 2026-09-20)

Kaynak: `docs/screenshot/stok/stok-*.png|webp` (git dışı). Zoho'da olmayan, kullanıcının mevcut ürünündeki Stok Yönetimi modülü; CRM'e uyarlanacak referanstır. Sağlık/eczane alanına özgü kavramlar (ilaç, ÜTS, ITS, eczane/ameliyathane depo tipleri) **alınmaz**; genel ticari stok kavramlarına çevrilir.

| Ekran | Görülen | CRM'e uyarlama |
|---|---|---|
| Genel Bakış | Karşılama + 6 adımlı kurulum sihirbazı (ayarlar, depo, birim, kategori, ilk ürün, ilk stok girişi); KPI kartları (toplam ürün, stok değeri, düşük stok, miadı yaklaşan, bugün giriş/çıkış); hareket trendi grafiği; kategori dağılımı; son hareketler; düşük stoklu ürünler; hızlı işlemler (stok giriş/çıkış) | Kurulum sihirbazı M7 "ilk kurulum" kartıyla aynı desen; KPI/grafik M9A widget kaydı ve M9F pano bileşenleri kullanılır. Miat/son kullanma: **v1 dışı** (parti/lot izleme sonra) |
| Depolar | Toplam/aktif/pasif/varsayılan kartları, tip çipleri, arama, "Yeni depo" diyaloğu (ad*, kod, tip*, sorumlu kişi, telefon, adres, not, aktif, varsayılan) | Depo CRUD; tip = kiracı tanımlı liste (sabit sağlık tipleri yok); sorumlu = kiracı üyesi (M9E `PhoneRule`, adres bloğu) |
| Kategoriler | Toplam/aktif/pasif/alt kategori, ürün tipi çipleri, "Yeni kategori" (ad*, kod, ikon, ürün tipi, üst kategori, renk paleti, açıklama, aktif) | Hiyerarşik ürün kategorisi; renk kapalı palet (M9B etiket paleti ile ortak); mevcut Commerce ürününe `categoryId` |
| Ürünler / Stok hareketleri / Uyarılar / Raporlar / Ayarlar | Yalnız menüde görünür (ekran görüntüsü yok) | Hareket defteri (giriş/çıkış/transfer/sayım düzeltmesi), eşik uyarıları, stok raporları, ayarlar (birimler, eşikler) |

**Karar (varsayılan):** stok, M9C'nin "stok yok" kapsam dışı kararını yumuşatan ayrı, plan bayraklı **opsiyonel modül** olur (`inventory` kapı modülü, kart C-M9K). M9C ürün/tedarikçi/satın alma emri üzerine kurulur; satın alma emri "teslim alındı" → stok girişi, satış siparişi/fatura → stok çıkışı (rezervasyon ve negatif stok politikası spec'te karara bağlanır). Önce spec yazılır; M9C merge olmadan Backend başlamaz.

...
4. **Görev işleyicileri** (`Crm.Worker/Workflows/ConductorTaskPollingService`): her görev türünü toplu poll eder, `WorkflowTaskRunner` ile sistem bağlamı + görev başına DI kapsamında çalıştırır. **Görev girdisi güvenilmezdir (C-SEC H1):** `input.tenantId`/`input.executionId` yalnız arama anahtarıdır; görev çalışmadan önce `(tenantId, executionId, Conductor'un verdiği workflow örnek kimliği)` üçlüsü `workflow_executions` ile doğrulanır (§16.1); iş kuralı hatası `FAILED_WITH_TERMINAL_ERROR` (yeniden denenmez, neden kararlı kod), geçici hata `FAILED` (motor yeniden dener). Görevler bir Worker'da sıralı işlenir (round-robin yarışsız). Conductor at-least-once teslim eder; onay kaydı `(execution, approver)` benzersiz olduğundan tekrar güvenlidir.
...
**Modül:** `Crm.Modules.Commerce.{Domain,Application,Contracts,Infrastructure,Api}`, şema `commerce`, tek `CommerceDbContext` (migration `InitialCommerce`). `Crm.Api` (`ModuleCatalog`), `Crm.Migrator` ve `Crm.Worker` (outbox: `QuoteAccepted`, `SalesOrderCreated`; M6A'da tüketici yok) Sales/Activities ile aynı kalıpla bağlandı. Ürün, teklif, sipariş ve numara sayacı aynı işlem sınırında yazıldığı için **tek modüldedir**. Diğer modüllere yalnız `Sales.Contracts` (`IRecordLookup`, yeni `IRecordRelationLookup`) ve `Identity.Contracts` (`IMemberLookup`, `TenantCalendarService`, `IAuditEntityPermissions`) üzerinden bağlıdır; Sales'in tablolarına doğrudan erişim yoktur ve fırsat aşaması otomatik değişmez.
...
**Modül:** `Crm.Modules.Service.{Domain,Application,Contracts,Infrastructure,Api}`, şema `service`, tek `ServiceDbContext` (migration `InitialService`: `dotnet dotnet-ef migrations add InitialService --project src/Modules/Service/Crm.Modules.Service.Infrastructure --startup-project src/Crm.Migrator --context ServiceDbContext -o Persistence/Migrations`). Sales'e yalnız `Sales.Contracts` (`IRecordLookup`, yeni `IContactAccountLookup`), Identity'ye `Identity.Contracts` (`IMemberLookup`, `ITenantDirectory`/`TenantCalendarService`, `IAuditEntityPermissions`, `OrganizationCreated`, `CrmPermissions.ReportsRead`) üzerinden bağlıdır (mimari testler yeşil). Rapor uçları aynı modüldedir (ayrı Reporting modülü yok).
...
**Testler:** `Crm.Modules.Service.Tests` — domain birim testleri (5×5 durum geçiş tablosu, çözüm notu, yeniden açma ve 14. gün sınırı, öncelik değişiminde SLA yeniden hesabı, ilk yanıt kuralı, SLA politikası doğrulaması) + Testcontainers HTTP testleri (sahte saatli host `ClockedHost`): numaralama (ardışık, 20 eşzamanlı POST boşluksuz, kiracı saat diliminde yıl sınırı, geri almada numara yanmaz, 10000. numara), tablo hücreleri HTTP üzerinden, SLA sınırları (`warnAt` tam eşik/bir tik önce, `now == dueAt` ihlal değil, +1 ms ihlal), politika anlık görüntüsü, süzgeç/özet/yanıt tutarlılığı, CRUD/firma-kişi kuralları, liste süzgeç/sıralama/sayfalama, zaman çizelgesi, eşzamanlı ilk yorumlar, izinler, kiracı izolasyonu (talep/firma/kişi/üye/SLA/sayaç/rapor/denetim), raporlar (bilinen veri kümesi, UTC+14 kiracı, varsayılan aralık), denetim + maskeleme + kayıt bazlı okuma, `CaseResolved` outbox, tohumlama yolları. `Crm.Tests.Architecture` ve `Crm.Tests.TenantIsolation` yeni modülü ve beş varlığı otomatik kapsar.
...
- Yeni modül `Service` (şema `service`, migration `InitialService`: `cases`, `case_comments`, `case_events`, `case_counters`, `sla_policies`); `Crm.Api` (`ModuleCatalog`), `Crm.Migrator`, `Crm.Worker`'a bağlandı; yeni test projesi `Crm.Modules.Service.Tests`; `CrmApiFactory` Respawn şemalarına `service` eklendi.
...
`packages.lock.json` tüm projelerde (`RestorePackagesWithLockFile`); CI ve Dockerfile'lar `dotnet restore --locked-mode`. **Yeni proje eklenince** (yeni modül) `dotnet restore Crm.slnx` ile kilit dosyası üretilip commit edilmelidir (kilitli kipte eksik dosya restore'u kırar). `NU1901–NU1904` yeniden hatadır (taramada zafiyetli paket çıkmadı). İmajlar: `postgres:17.11-alpine`, `redis:7.4.11-alpine`, `aspnet:10.0.12` (SDK 10.0.401 ile aynı çalışma zamanı); `web/Dockerfile` (node/nginx) bu kartın kapsamı dışındaydı.

### 16.6 Testler
`Crm.Modules.Identity.Tests`: `AccountModelApiTests` (geçici parola/zorunlu değişim, parola değiştirme + tüm oturumların iptali, bekleyen davet/sızıntı/başkası kabul edemez/reddetme, politika), `LoginAndSessionHardeningApiTests` (genel hata sırası, IP+hesap/e-posta azaltma, eşzamanlı refresh, tolerans/hırsızlık, mutlak ömür, `kid`/`alg`, hash yükseltme, M2, Kestrel sınırı, sayfalama), `DelegationAndCacheApiTests` (M7, M9); `Crm.Modules.Workflows.Tests.TaskInputTrustApiTests` (H1, L3); `Crm.Modules.Activities.Tests.RelatedNameVisibilityApiTests` (L2); `Crm.Modules.Sales.Tests.AccountWebsiteApiTests` (L7); `Crm.Shared.Infrastructure.Tests.GridLikeEscapingTests` (L8); `Crm.Tests.Architecture.RequestAuthorizationTests` (M6).

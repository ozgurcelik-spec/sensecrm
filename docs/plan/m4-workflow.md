...
- Workflow tanımları kodda (JSON, sürümlü) tutulur, API başlangıcında Conductor'a idempotent kaydedilir. Görev işleyicileri (worker) `Crm.Worker` içinde Conductor'ı poll eder; her görev `TenantScope` içinde çalışır (kiracı `input.tenantId` ile gelir).

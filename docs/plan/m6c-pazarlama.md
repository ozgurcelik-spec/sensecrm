...
## Modül kararı
- Yeni modül `Sense.Crm.Modules.Marketing.{Domain,Application,Contracts,Infrastructure,Api}` (şema `marketing`, tek `MarketingDbContext`, migration `InitialMarketing`). `./build/new-module.ps1 -Name Marketing` ile açılır; `ModuleCatalog`, `Sense.Crm.Migrator`, `Sense.Crm.Worker` (outbox + `LeadConverted` tüketicisi) Sales/Activities ile aynı kalıpla bağlanır.
...
## İzinler
- Yeni `Sense.Crm.Modules.Marketing.Contracts.MarketingPermissions` (`Module = "marketing"`, `Group = "crm"`): `crm.campaigns.read`, `crm.campaigns.write`; modül `IModule.Permissions` ile kataloğa bildirir → `GET /permissions` 18 → **20** anahtar (mevcut anahtar sayısını sabitleyen testler güncellenir). `crm.reports.read` (Identity.Contracts, ortak) rapor ucu için kullanılır, yeni rapor izni yok.
...
## Testler
**Backend** (`Sense.Crm.Modules.Marketing.Tests`; birim + Testcontainers HTTP, Respawn şemalarına `marketing` eklenir):
...
**Merge kapısı (pano kuralı):** `dotnet build Sense.Crm.slnx --no-incremental` 0 uyarı, `dotnet format --verify-no-changes`, `dotnet test Sense.Crm.slnx`, mimari + kiracı izolasyon testleri, yeni her kiracı varlığı için çapraz-kiracı testi, canlı duman testi (kart portu/veritabanı `crm_marketing`, örn. API 5083): kampanya aç → lead'leri toplu ekle → lead'i dönüştür → üyelik `converted` + metrikler/rapor doğrulanır.

## Uygulama notları
- **Sıcak dosyalar (yalnız ekleme):** `ModuleCatalog.cs` (`new MarketingModule()`), `Sense.Crm.Migrator/Program.cs`, `Sense.Crm.Worker/Program.cs` (Marketing outbox + `LeadConverted` işleyici kaydı + `AddMarketingContractServices`), `TestFixture.cs` (Respawn `marketing`), `SharedResource{,.en}.resx`, `docs/architecture/backend.md` (Marketing bölümü), web sıcak dosyaları (yukarıda), `web/README.md`. `Identity.Contracts/Permissions.cs` ve `SystemRoleDefinitions.cs` **değişmez**.
- Migration: `dotnet dotnet-ef migrations add InitialMarketing --project src/Modules/Marketing/Sense.Crm.Modules.Marketing.Infrastructure --startup-project src/Sense.Crm.Migrator --context MarketingDbContext -o Persistence/Migrations`. Tüm indeksler `tenant_id` ile başlar (K3).

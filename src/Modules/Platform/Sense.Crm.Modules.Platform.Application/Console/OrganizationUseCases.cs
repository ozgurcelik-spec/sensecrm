    ...
    TimeProvider clock) : IQueryHandler<GetOrganizationQuery, OrganizationDetailDto>
{
    public async Task<Result<OrganizationDetailDto>> Handle(GetOrganizationQuery query, CancellationToken cancellationToken)
    {
        ...
        return new OrganizationDetailDto(
            ...
            row.CreatedAt,
            row.Usage,
            new EffectiveLimitsDto(effective.MaxUsers, EntitlementMath.FiniteRecords(effective.MaxRecords), effective.Modules, effective.MaxWebhooks, effective.MaxApiKeys, effective.MaxStorageMb),
            overrides,
            planChangedAt,
            suspension,
            deletion);

...
public sealed class OnionArchitectureTests
{
    ...
    private const string ModulesPrefix = "Sense.Crm.Modules.";
    ...
    private const string KernelAssembly = "Sense.Crm.Shared.Kernel";
    private const string SharedContracts = "Sense.Crm.Shared.Contracts";
    private const string SharedInfrastructure = "Sense.Crm.Shared.Infrastructure";
    private const string SharedWeb = "Sense.Crm.Shared.Web";
    ...
    public void Controllers_InheritApiControllerBase()
    {
        var result = Types.InAssemblies(Named(ApiSuffix))
            ...
            .Should().Inherit(typeof(Sense.Crm.Shared.Web.Controllers.ApiControllerBase))
            .GetResult();
        result.IsSuccessful.ShouldBeTrue(string.Join(", ", result.FailingTypeNames ?? []));
    }
    ...
    public void PermissionKeys_FollowNamingConvention()
    {
        ...
        var violations = Named(ApplicationSuffix)
            ...
            .SelectMany(t => t.GetCustomAttributes(typeof(Sense.Crm.Shared.Contracts.Security.RequiresPermissionAttribute), inherit: false)
                .Cast<Sense.Crm.Shared.Contracts.Security.RequiresPermissionAttribute>()
    ...
    public void PermissionCatalogs_UseContractKeyFormatAndGroups()
    {
        ...
        var permissions = Named(ContractsSuffix)
            ...
            .Where(p => p is not null && typeof(IEnumerable<Sense.Crm.Shared.Contracts.Security.Permission>).IsAssignableFrom(p.PropertyType))
            .SelectMany(p => (IEnumerable<Sense.Crm.Shared.Contracts.Security.Permission>)p!.GetValue(null)!)
            .ToList();
    ...
    {
        var dbContextType = typeof(Sense.Crm.Shared.Infrastructure.Persistence.ModuleDbContext);
    ...
    {
        // Test projesi Sense.Crm.Api'ye referans verir; tüm Sense.Crm.* assembly'leri çıktı klasöründe bulunur.

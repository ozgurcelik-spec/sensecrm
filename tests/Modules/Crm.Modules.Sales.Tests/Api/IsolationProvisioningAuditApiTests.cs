...
public sealed class IsolationProvisioningAuditApiTests(CrmApiFactory factory)
{
    ...
    public async Task SignUp_SeedsTheDefaultPipeline_ThroughTheOutboxEvent_InTheOrganizationLanguage(string locale, string first, string won, string lost)
    {
        ...
        pipeline.Stages.Single(s => s.Kind == Crm.Modules.Sales.Domain.Pipelines.StageKind.Won).Name.ShouldBe(won);
        pipeline.Stages.Single(s => s.Kind == Crm.Modules.Sales.Domain.Pipelines.StageKind.Lost).Name.ShouldBe(lost);
        ...
        {
            (await scope.ServiceProvider.GetRequiredService<Crm.Modules.Sales.Application.IDefaultPipelineSeeder>().EnsureAsync(org.TenantId, locale, Ct)).ShouldBeFalse();
        }
    ...
    private async Task<List<Crm.Modules.Sales.Domain.Pipelines.Pipeline>> PipelinesOfAsync(Guid tenantId)
    {
        using var scope = factory.Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<SalesDbContext>();

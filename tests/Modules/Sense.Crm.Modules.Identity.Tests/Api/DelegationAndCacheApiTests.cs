...
public sealed class DelegationAndCacheApiTests(CrmApiFactory factory)
{
    ...
    public async Task AcceptingAnInvitation_GrantsThePermissionsImmediately()
    {
        ...
        var switched = (await (await guest.PostAsJsonAsync($"{Base}/auth/switch-organization", new { organizationId = orgId }, Ct)).Content.ReadFromJsonAsync<Sense.Crm.Modules.Identity.Application.AuthResponse>(Ct))!;
